...
RSpec.describe InfoRequest do
  ...
  describe '#law_used' do
    ...
    context 'with public body' do
      ...
      let(:eir) { FactoryBot.build(:public_body) }

      before do
        allow(foi).to receive(:legislation).and_return(
          FactoryBot.build(:legislation, key: 'foi')
        )

        allow(eir).to receive(:legislation).and_return(
